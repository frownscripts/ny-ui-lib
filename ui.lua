...
function lib:Destroy()
	pcall(function()
		for _,c in ipairs(LibraryFunctions.Connections or {}) do
...
function lib:Create(ver, size, hidekey)
	...
	-- Auto scale for phone/tablet/pc
	...
	local function applyScale()
		...
		local w = (MainFrame.Size.X.Offset > 0) and MainFrame.Size.X.Offset or 700
		local h = (MainFrame.Size.Y.Offset > 0) and MainFrame.Size.Y.Offset or 625
		local margin = 24
		local sx = (vp.X - margin) / w
		local sy = (vp.Y - margin) / h
		local sca = math.min(sx, sy, 1)
		uiScale.Scale = math.clamp(sca, 0.55, 1)
	end
	applyScale()
	pcall(function()
		local cam = workspace.CurrentCamera
	...
	PromptYesButton.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
	...
	PromptYesButton.InputEnded:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
	...
	PromptNoButton.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
	...
	PromptNoButton.InputEnded:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
	...
	function tabs:Tab(title, AssID, desc1, desc2)
		...
		function subtabs:SubTab(title, AssID)
			...
			function items:Toggle(side, text, config, callback)
				...
				ToggleInteract.Activated:Connect(function()
					if toggled == false then
						toggled = true
			...
			function items:Colorpicker(side, text, preset, callback)
				...
				Color.InputBegan:Connect(function(input)
					if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
						...
						ColorInput = RunService.RenderStepped:Connect(function()
							local mousePos = LibraryFunctions:GetMouseLocation()
				...
				Colorpicker.Activated:Connect(function()
			...
			function items:Button(side, text, callback, callback_2, prompttitle, promptmessage)
				...
				ButtonFrame.Activated:Connect(function()
					if promptmessage ~= nil then
						...
						c_yes = PromptYesButton.Activated:Connect(function()
							c_yes:Disconnect()
							c_no:Disconnect()
							PromptContainer.Visible = false
							callback()
						...
						c_no = PromptNoButton.Activated:Connect(function()
							c_yes:Disconnect()
							c_no:Disconnect()
							PromptContainer.Visible = false
							callback_2()
			...
			function items:Bind(side, text, mode, callback)
				...
				Interaction.Activated:Connect(function()
					if binding then return end
				...
				local function dropdownoff()
					if DropdownChildFrame.Visible == true then
						...
						for i,v in next, DropdownChildFrameScroll:GetDescendants() do
							...
							if v:IsA('ImageLabel') then
								TweenService:Create(v, TweenInfo.new(.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {ImageTransparency = 1}):Play()
							end
						end
						tween:Play()
						opened = false
...
-- Example usage has been moved to `kkexample.lua`.
getgenv().library = lib
return lib
